...
function RouteComponent() {
  ...
  return (
    <div className="flex-1 flex justify-center items-center">
      <div className="md:w-[60vw] sm:w-[70vw] w-[95vw] p-7 bg-white rounded-xl space-y-3">
        <Tabs defaultValue="create">
          <TabsList className="h-10">
            <TabsTrigger value="create" className="text-md px-4">
              Create
            </TabsTrigger>
            <TabsTrigger value="join" className="text-md px-4">
              Join
            </TabsTrigger>
          </TabsList>
      ...
      <Dialog
        open={openRoomScheduledDialog}
        onOpenChange={(open) => setOpenRoomScheduledDialog(open)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Room Scheduled</DialogTitle>
            <DialogDescription>
              You just scheduled a room. Connect at specified date and time
              using the link below or join using the room ID.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2 text-lg">
            <p>
              <span className="font-semibold">Room name:</span>{' '}
              {scheduledRoomData?.['roomName']}
            </p>
            <p>
              <span className="font-semibold">Scheduled at:</span>{' '}
              {toLocalDate(scheduledRoomData?.['startAt'])}
            </p>
            <p>
              <span className="font-semibold">Room ID:</span>{' '}
              {scheduledRoomData?.['roomId']}
            </p>
      ...
      <Dialog
        open={openDenyRoomJoinDialog}
        onOpenChange={(open) => setOpenDenyRoomJoinDialog(open)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Cannot Join Room</DialogTitle>
            <DialogDescription>
              This room is scheduled. Connect at specified date and time using
              the link below or join using the room ID.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2 text-lg">
            <p>
