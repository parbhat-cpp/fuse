...
export class YtService {
  ...
  async setVideo(
    ...
    const { exists, isMember, roomData, roomType } =
      await this.roomService.roomExists(roomId, userId);
    ...
    for (let i = 0; i < roomData.attendeesId.length; i++) {
      ...
      client.to(userIdToSocketId.get(attendeeId)).emit(YTEvents.SET_VIDEO_ID, {
        id: 'youtube',
        name: 'YouTube',
        videoId,
        username,
  ...
  async playVideo(
    ...
    const { exists, isMember, roomData } = await this.roomService.roomExists(
      roomId,
      userId,
    );
    ...
    for (let i = 0; i < roomData.attendeesId.length; i++) {
      ...
      client
        ...
        .emit(YTEvents.SET_VIDEO_PLAY, username);
    }
  }
  ...
  async pauseVideo(
    ...
    for (let i = 0; i < roomData.attendeesId.length; i++) {
      ...
      client
        .to(userIdToSocketId.get(attendeeId))
        .emit(YTEvents.SET_VIDEO_PAUSE, username);
    }
  }
  ...
  async seekVideo(
    ...
    for (let i = 0; i < roomData.attendeesId.length; i++) {
      ...
      client
        .to(userIdToSocketId.get(attendeeId))
        .emit(YTEvents.SEEK_VIDEO_TO, { username, position });
    }
  }
}
